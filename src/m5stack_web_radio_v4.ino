//***************************************************************************************************
//*  M5Stack Radio -- Webradio receiver for M5Stack, VS1053 MP3 module and display.                 *
//***************************************************************************************************

/*
 * Aug 9 2019: 
 *   Rotary encoder removed.
 *   Speaker noise killed.
 *   Moved VS1053 code to included .h & .cpp files.
 *   
 * Aug 10 2019:  
 *   Fixed info display
 *   
 * Aug 11 2019:  
 *   Fixed display of battery level and volume
 *   
 * Dec 28 2019:
...
#define M5STACK_DISPLAY                                 // M5Stack display
// #define DUMMYTFT

#define sv DRAM_ATTR static volatile

// 0-100, set to 90-95 to prevent distortion
#define VS1053_DEFAULT_VOLUME 92
...
// Accesspoint name if connection to WiFi network fails.  Also the hostname for WiFi and OTA.
// Note that the password of an AP must be at least as long as 8 characters.
// Also used for other naming.
#define NAME "M5StackRadio"
...
// "M5StackRadio/command" if mqttprefix is "M5StackRadio".
...
// be read from these preferences.                                                                 *
// Items in ini_block can be changed by commands from webserver/MQTT/Serial.                       *
//**************************************************************************************************

enum display_t  { T_UNDEFINED, T_DUMMYTFT, T_M5STACK_DISPLAY // Various types of display
                } ;
...
struct progpin_struct                                    // For programmable input pins
{
  ...
  String         command ;                               // Command to execute when activated, example: "uppreset=1"
...
progpin_struct   progpin[] =                             // Input pins and programmed function
{
  ...
  //{ 16, false, false,  "", false },                    // DREQ for VS1053
  ...
  { 37, false,  false,  "", false },                     // button C, internal pull-up
  { 38, false,  false,  "", false },                     // button B, internal pull-up
  { 39, false,  false,  "", false },                     // button A,  internal pull-up
...
// for dual digital potmeter
int8_t  ddp_cs_pin = 12;
int8_t  ddp_shutdown_pin = -1;
...
// The object for the dual digital potmeter
DualDigitalPotmeter* ddp ;
...
#include "m5stack_display.h"                             // For M5STACK_DISPLAY 320x240 display
#endif
...
bool connecttohost()
{
  ...
  tftset ( 0, "M5Stack Radio" ) ;                   // Set screen segment text top line
...
bool connecttofile()
{
  ...
  tftset ( 0, "M5Stack MP3 Player" ) ;                      // Set screen segment top line
  displaytime ( "" ) ;                                    // Clear time on TFT screen
...
String readprefs ( bool output )
{
  ...
  if ( i == 0 )
  {
    outstr = String ( "No preferences found.\n"
                      "Use defaults or run M5Stack_radio_init first.\n" ) ;
  }
  return outstr ;
}
...
void setup()
{
  ...
  dbgprint ( "Starting M5Stack Radio running on CPU %d at %d MHz.  Version %s.  Free memory %d",
             xPortGetCoreID(),
             ESP.getCpuFreqMHz(),
             VERSION,
             ESP.getFreeHeap() ) ;                       // Normally about 170 kB
#if defined ( M5STACK_DISPLAY )                                // Report display option
  dbgprint ( dtyp, "M5STACK_DISPLAY" ) ;
#endif
#if defined ( DUMMYTFT )
  ...
  ddp = new DualDigitalPotmeter ( ddp_cs_pin, ddp_shutdown_pin, SPI_activated);
  claimSPI("ddp");
  ddp->begin(); 
  ddp->setVolume(0);
  //delay(10);
       ...
       ( ini_block.tft_scl_pin >= 0 ) )
  {
    ...
    {
      //dsp_setRotation() ;                              // Use landscape format, not needed for M5Stack
  ...
  vs1053player->setVolume(VS1053_DEFAULT_VOLUME);
  delay(10);
...
void loop()
{
  ...
  handleVolPub() ;                                  // See if time to publish volume                                       // Check rotary encoder functions
}
...
const char* analyzeCmd ( const char* par, const char* val )
{
  ...
  if ( argument.indexOf ( "volume" ) >= 0 )           // Volume setting?
  {
    ...
    oldvol = ddp->getVolume() ;                       // Get current volume
...
void displayinfo ( uint16_t inx )
{
  ...
  if ( dsp_tft )                                           // TFT active?
  {
    ...
    if ( ( dsp_getheight() > 64 ) && ( p->y > 1 ) )        // Need space for divider?
    {
    ...
    if ( xQueueReceive ( dataqueue, &inchunk, 5 ) )
    {
      ...
      switch ( inchunk.datatyp )                                    // What kind of chunk?
      {
        ...
        case QSTOPSONG:
          ...
          mqttpub.trigger ( MQTT_PLAYING ) ;                        // Request publishing to MQTT
          delay(10);
          claimSPI ( "ddp stopsong" ) ;                             // Claim SPI bus
          //vs1053player->setVolume ( 0 ) ;                           // Mute
          ddp->setVolume(0);
          releaseSPI() ;                                            // Release SPI bus
          delay(10);
          claimSPI ( "vs1053 stopsong" ) ;                                 // Claim SPI bus
...
void handle_spec()
{
  ...
  {
    delay(10);
    claimSPI ( "ddp mute" ) ;                                 // Claim SPI bus
    //vs1053player->setVolume ( 0 ) ;                         // Mute
    ddp->setVolume(0);
    releaseSPI() ;                                            // Release SPI bus
    delay(10);
  ...
  {
    delay(10);
    claimSPI ( "ddp unmute" ) ;                               // Claim SPI bus
    //vs1053player->setVolume ( ini_block.reqvol ) ;            // Unmute
    ddp->setVolume ( ini_block.reqvol ) ;                     // Unmute
    releaseSPI() ;                                            // Release SPI bus
    delay(10);
  }
  ...
  {
    reqtone = false ;
    claimSPI ( "vs1053_settone" ) ;                           // Claim SPI bus
